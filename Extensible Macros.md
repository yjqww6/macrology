...
这样，`use-expander`的`x`被打上了唯一的 _macro-introduction scope_ ，`expander1`的`x`不可能具有该 _scope_ ，不会意外的绑定。
...
`syntax-local-identifier-as-binding`可以消除 _use-site scope_ ，所以如果事先知道结果的形状，还是有可能解决问题的：
